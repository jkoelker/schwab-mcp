...
class TestBuildEquityOrderSpec:
    ...
    def test_invalid_order_type(self, symbol, quantity):
        with pytest.raises(ValueError, match="Invalid order_type: TRAILING_STOP. Must be one of"):
    ...
    def test_invalid_instruction(self, symbol, quantity, order_type):
        ...
        with pytest.raises(ValueError, match=f"Invalid instruction for {order_type} order"):
            _build_equity_order_spec(
                symbol,
                quantity,
                "HOLD",
...
class TestBuildOptionOrderSpec:
    ...
    def test_invalid_order_type(self, symbol, quantity):
        with pytest.raises(ValueError, match="Invalid order_type: STOP. Must be one of"):
...
class TestBuildTrailingStopOrderSpec:
    ...
    def test_trailing_stop_value_valid(self, symbol, quantity, instruction):
        result = _build_trailing_stop_order_spec(symbol, quantity, instruction, trail_offset=5.0, trail_type="VALUE")
        spec = cast(dict[str, Any], result.build())
    ...
    def test_trailing_stop_percent_valid(self, symbol, quantity, instruction):
        result = _build_trailing_stop_order_spec(symbol, quantity, instruction, trail_offset=5.0, trail_type="PERCENT")
        spec = cast(dict[str, Any], result.build())
    ...
    def test_trailing_stop_case_insensitive(self, symbol, quantity):
        result = _build_trailing_stop_order_spec(symbol, quantity, "sell", trail_offset=10.0, trail_type="percent")
        spec = cast(dict[str, Any], result.build())
    ...
    def test_invalid_trail_type(self, symbol, quantity):
        with pytest.raises(ValueError, match="Invalid trail_type: TICK"):
            _build_trailing_stop_order_spec(symbol, quantity, "SELL", trail_offset=5.0, trail_type="TICK")
    ...
    def test_invalid_trail_offset(self, symbol, quantity, bad_offset):
        with pytest.raises(ValueError, match="trail_offset must be positive"):
            _build_trailing_stop_order_spec(symbol, quantity, "SELL", trail_offset=bad_offset)
